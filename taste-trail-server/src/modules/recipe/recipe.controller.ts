...
const getAllRecipesForMealPlan = catchAsync(
...
const recommendedRecipes = catchAsync(async (req: Request, res: Response) => {
  const user = req.user;
  const result = await recipeServices.recommendedRecipes(user?.email);
  responser(res, {
    statusCode: 200,
