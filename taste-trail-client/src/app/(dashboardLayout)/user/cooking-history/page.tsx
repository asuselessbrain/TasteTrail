  ...
  return (
    <div className="min-h-screen p-8">
      <div className="max-w-360 mx-auto">
        <div className="mb-12">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Cooking History</h1>
          <p className="text-lg text-gray-600">Your culinary journey and achievements</p>
        ...
        {!historyList || historyList.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16">
            <div className="text-center">
              <p className="text-xl text-gray-500 mb-6">No cooking history yet</p>
              <Link href="/user/meal-planner">
                <Button className="bg-orange-500 hover:bg-orange-600">Start Planning Meals</Button>
              </Link>
            </div>
          </div>
        ) : (
          ...
          <div className="space-y-6">
            {historyList.map((history: any, index: number) => {
              ...
              return (
                <div key={history._id} className="relative pl-8 pb-8 border-l-2 border-orange-300 last:border-l-0">
                  {/* Timeline Dot */}
        ...
        {historyList && historyList.length > 0 && (
          <div className="mt-12 bg-white rounded-lg shadow-md p-6">
            <h3 className="text-xl font-bold text-gray-900 mb-4">Your Cooking Stats</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="text-center p-4 bg-orange-50 rounded-lg">
                <p className="text-3xl font-bold text-orange-600">{historyList.length}</p>
                <p className="text-sm text-gray-600">Recipes Cooked</p>
              </div>
              <div className="text-center p-4 bg-amber-50 rounded-lg">
                <p className="text-3xl font-bold text-amber-600">
                  {(() => {
                    const totalMinutes = historyList.reduce((sum: number, h: any) => sum + (h.recipeId?.cookingTime || 0), 0);
              ...
              <div className="text-center p-4 bg-purple-50 rounded-lg">
                <p className="text-3xl font-bold text-purple-600">
                  {new Set(historyList.map((h: any) => h.recipeId?.cuisineId?.name).filter(Boolean)).size}
                </p>
